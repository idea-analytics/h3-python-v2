...
# -----------------------------
def load_hex_data(file_path='hex_data.json'):
    """Load pre-processed hex data from JSON file"""
    try:
        with open(file_path, 'r') as f:
            data = json.load(f)
        return data
    except FileNotFoundError:
        print(f"Error: {file_path} not found. Please run the data processor first.")
        return None
...
def server(input, output, session):
    ...
    def load_data():
        ...
        data = load_hex_data('hex_data.json')
        hex_data.set(data)
        if data:
    ...
    def map_plot():
        ...
        if not data:
            return ui.div(
                ...
                ui.p("Please ensure 'hex_data.json' is in the app directory.", 
...
1. Run data_processor.py locally to generate hex_data.json:
   python data_processor.py

2. Upload both files to your Posit Connect deployment:
   - app.py (this file)
   - hex_data.json (generated by data_processor.py)
...
4. The app will automatically load hex_data.json on startup
